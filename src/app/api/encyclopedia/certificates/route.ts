...
export async function GET(request: NextRequest) {
    try {
        ...
        else return apiError(ERROR_CODES.BAD_REQUEST, 'visitorId or userId required', 400);

        const certificates = await EncyclopediaCertificate.find(query)
            .sort({ completedAt: -1 })
            .lean();

        return apiSuccess(certificates, 'Certificates fetched successfully');
    } catch (error) {
        console.error('Error fetching certificates:', error);
        return apiError(ERROR_CODES.ENCYCLOPEDIA_FETCH_FAILED, 'Failed to fetch certificates', 500);
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!sectionId || !userName) {
            return apiError(ERROR_CODES.VALIDATION_FAILED, 'sectionId and userName required', 400);
        }
        ...
        if (existingCert) {
            return apiSuccess(existingCert, 'Certificate already exists');
        }
        ...
        return apiSuccess(certificate, 'Certificate created successfully', 201);
    } catch (error) {
        console.error('Error creating certificate:', error);
        return apiError(ERROR_CODES.ENCYCLOPEDIA_CREATE_FAILED, 'Failed to create certificate', 500);
    }
}
