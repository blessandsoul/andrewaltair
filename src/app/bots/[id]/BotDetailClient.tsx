...
// MAIN PAGE COMPONENT
// ═══════════════════════════════════════════════════════════════════════════
export default function BotDetailClient({ initialBot }: { initialBot?: AIBot | null }) {
    const params = useParams();
    const router = useRouter();

    // Seeded from the server page so name/description/features are in the
    // initial SSR HTML (used to be fetch-on-mount = invisible to crawlers)
    const [bot, setBot] = useState<AIBot | null>(initialBot ?? null);
    ...
    // Bot core data: skip the first fetch when server-seeded.
    const seededRef = useRef(Boolean(initialBot));
    useEffect(() => {
        if (!params.id) return;
    ...
    // Secondary data keys off the loaded bot's real Mongo id — the URL param
    // may be a codename slug, which the /api/bots/[id]/* sub-routes don't accept.
    const botMongoId = bot?.id;
    useEffect(() => {
        if (!botMongoId) return;
        fetchVersions(); // New
        fetchComments();
        checkPurchaseStatus();
    ...
    const checkWishlistStatus = async () => {
        try {
            ...
            const response = await fetch(`/api/bots/${bot?.id || params.id}/wishlist`);
    ...
    const toggleWishlist = async () => {
        try {
            const response = await fetch(`/api/bots/${bot?.id || params.id}/wishlist`, { method: 'POST' });
            if (response.ok) {
                const data = await response.json();
                setIsWishlisted(data.isWishlisted);
            }
    ...
    const fetchVersions = async () => {
        try {
            const response = await fetch(`/api/bots/${bot?.id || params.id}/versions`);
            const data = await response.json();
    ...
    const checkPurchaseStatus = async () => {
        try {
            ...
            const response = await fetch(`/api/bots/${bot?.id || params.id}/check-purchase`, { headers });
            const data = await response.json();
    ...
    const fetchComments = async () => {
        try {
            const response = await fetch(`/api/bot-comments?botId=${bot?.id || params.id}`);
            const data = await response.json();
    ...
    const handleSubmitComment = async () => {
        ...
        try {
            const response = await fetch('/api/bot-comments', {
                ...
                body: JSON.stringify({ botId: bot?.id || params.id, text: newComment, rating })
            });
            if (response.ok) {
                setNewComment('');
                setRating(5);
