...
export default function BotsPageClient({ initialBots }: { initialBots?: AIBot[] }) {
    // State — seeded from the server page so the full bot grid is present in
    // the initial SSR HTML (this page used to be fetch-on-mount = invisible to crawlers)
    const [bots, setBots] = useState<AIBot[]>(initialBots ?? []);
    const [isLoading, setIsLoading] = useState(!(initialBots && initialBots.length > 0));
    ...
    // Skip the first client fetch when the server already seeded the grid;
    // any filter/search change recreates fetchBots and refetches as before.
    const seededRef = useRef(Boolean(initialBots && initialBots.length > 0));
    useEffect(() => {
